...
section Initialization
  ...
  source files. -/
  partial def leanpkgSetupSearchPath (m : DocumentMeta) (imports : Array Import) (hOut : FS.Stream) : IO SearchPath := do
  ...
  def compileHeader (m : DocumentMeta) (hOut : FS.Stream) : IO (Snapshot × SearchPath) := do
    ...
    if (← fileExists "leanpkg.toml" <&&> fileExists s!"{← appDir}/leanpkg") then
      let pkgSearchPath ← leanpkgSetupSearchPath m (Lean.Elab.headerToImports headerStx).toArray hOut

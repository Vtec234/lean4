...
@[init mkHandlersRef]
constant handlersRef : IO.Ref (List Handler) := arbitrary
  ...
  | `(<$n>$cs*</$m>) => -- {{{
    if n.getId == m.getId then do
      let cs ← cs.mapM fun c => match c with
        | `(child|$t:text)    => pure $ quote (t.getArg 0).getAtomVal!
        | `(child|{$t})       => pure t
...
macro v:verb p:path " => " t:term : command => do -- {{{
  ...
  let t ← p.getArgs.foldrM (fun pi t => match pi with
    | `(pathItem|$l:pathLiteral) => `(do checkPathLiteral $(quote (l.getArg 0).getAtomVal!); $t:term)
    | `(pathItem|{$id}) => `(do let $id:ident ← getPathPart; $t:term)
    | _ => unreachable!) t
  `(def handler : Handler := do
      checkVerb $(quote (v.getArg 0).getAtomVal!)
      $t:term
